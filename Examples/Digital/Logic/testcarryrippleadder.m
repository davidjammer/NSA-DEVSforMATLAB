...
AdderSout = devs(toworkspace("AdderSout", "addersOut", 0, [0, rOut]));
AdderCout = devs(toworkspace("AdderCout", "addercOut", 0, [0, rOut]));

Dectobin1 = devs(dectobin8("dectobin1",[0, 1],mdebug));
Dectobin2 = devs(dectobin8("dectobin2",[0, 1],mdebug));
Adder = carryrippleadder8bitCM("Adder",[0, 1],mdebug);
Bintodec = devs(bin8todec("bintodec",[0, 1],mdebug));
...
N1.add_coupling("Adder","c","AdderCout","in");
...
subplot(5,1,5)
stairs(simout.addercOut.t,simout.addercOut.y,'-*');
title("Adder c");
xlim([0,16])
ylim([0,1.3])