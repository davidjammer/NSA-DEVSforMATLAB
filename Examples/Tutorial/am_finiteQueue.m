classdef am_finiteQueue < handle
  %% Description
  %  FIFO queue with finite capacity and output of current queue length
  %% Ports
  %  inputs:
  ...
  %    nq      queue length
  %    isFull  true, if queue length = capacity
  %% States
  %  s: emptyFree|emptyBlocked|queuingFree|queuingBlocked
  %  q: vector of queued entities
  %% System Parameters
  %  name:      object name
  %  capacity:  maximal number of entities in queue
  ...
  methods
    function obj = am_finiteQueue(name, capacity, tD, tau, debug)
      obj.s = "emptyFree";
      obj.q = [];
      obj.name = name;
          ...
          if isempty(x)     % internal event
            ...
            if ~isempty(bl) && bl == "1"
              ...
              if length(obj.q) < obj.capacity
                ...
                % hard to get here - but you never know :-)
