function out = testServerExp(showPlot)
  if nargin == 0
    showPlot = false;
  end
  ...
  rng(2);     % set seed for random generator
	...
	load_system("ServerExp_Model");
	model_generator("ServerExp_Model");
	out = model_simulator("ServerExp_Model", tEnd);

  if showPlot
    figure("name", "testServerExp", "NumberTitle", "off")
    subplot(3,2,1)
